...
SYSTEM_PROMPT = (
    ...
    "MASS-DEPENDENT DEVOTIONS: if a recurring devotion is described in the bulletin "
    "as happening 'after Mass' / 'following Mass' / 'before Mass' (e.g. 'Perpetual "
    "Novena after 10am Monday Mass, followed by Benediction at 11am', 'Rosary "
    "before the 9:30 Mass') AND the bulletin cancels that Mass on a specific day "
    "('No Mass today', 'No Mass on Wednesday'), then DO NOT emit the dependent "
    "devotion on that day either. The devotion only happens when its parent Mass "
    "does. This applies even if the devotion has its own concrete clock time "
